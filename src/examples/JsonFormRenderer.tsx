...
import { Button, Card, CardBody, CardHeader, Textarea, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, useDisclosure, Divider } from '@heroui/react';
...
export function JsonFormRenderer() {
  ...
  const { isOpen: isSubmissionModalOpen, onOpen: onSubmissionModalOpen, onOpenChange: onSubmissionModalOpenChange } = useDisclosure();
  ...
  const handleParseJson = () => {
    try {
      ...
      if (!parsed.metadata || !parsed.settings || !parsed.layout || !parsed.fields || !parsed.fieldMap) {
        throw new Error('Invalid form JSON structure. Missing required properties.');
      }
      
      setParsedForm(parsed);
    } catch (error) {
      setParseError(error instanceof Error ? error.message : 'Invalid JSON format. Please check your syntax.');
      setParsedForm(null);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto p-6">
                ...
                <CardBody>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{parsedForm.metadata.title}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-default-600">Fields:</span>
                      <span className="font-medium">{parsedForm.fields.length}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-default-600">Required Fields:</span>
                      <span className="font-medium">{parsedForm.validation.requiredFields.length}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-default-600">Version:</span>
                      <span className="font-medium">{parsedForm.metadata.version}</span>
                    </div>
                  </div>
                </CardBody>
              </Card>
              ...
              <CardBody className="p-0">
                {parsedForm ? (
                  ...
                  <div className="text-center py-12 text-default-500">
                    ...
                    <p>Upload a JSON file, paste JSON data, or load a sample to preview the form</p>
                  </div>
                )}
              </CardBody>
            </Card>
        ...
        <Modal isOpen={isSubmissionModalOpen} onOpenChange={onSubmissionModalOpenChange} size="2xl">
          <ModalContent>
            {(onClose) => (
              <>
                ...
                <ModalBody>
                  <div className="space-y-4">
                    ...
                    <div className="text-sm text-default-600">
                      <strong>Note:</strong> This is a demonstration. In a real application, this data would be sent to your server or API endpoint.
                    </div>
                  </div>
                </ModalBody>
                <ModalFooter>
